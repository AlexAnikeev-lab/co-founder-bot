...
async def cmd_start(
    ...
    try:
        ...
        # AuthMiddleware может пропустить загрузку user на RegistrationStates ради скорости.
        # Для /start нам важно определить реальный статус регистрации, поэтому при отсутствии user
        # делаем один аккуратный запрос в БД (в рамках одной session от DbSessionMiddleware).
        ...
        if message.from_user:
            try:
                u = await UserRepository.get_by_telegram_id(session, message.from_user.id)
                if u:
